...
#get parameters
while getopts 'i:o:m:n:d:a:h' opt; do
	case $opt in
		i) input_file="$OPTARG";;
		o) output_dir="$OPTARG";;
...
determ_samp_dep(){
	...
	cd ../../ && mv ${table_dir} table.qza.unzip
...
if [ $? -eq 0 ];then
        ...
        step6="error happened at when determinging sampling depth, please check!"
fi
mv ${denoise_dir}/${table_dir} table.qza.unzip
cd ${output_dir}


#Alpha and beta diversity analysis
echo "performing alpha diversity analysis ..."
if [ ! -d ${denoise_dir}/core-metrics-results ];then
	qiime diversity core-metrics-phylogenetic \
		--i-phylogeny ${denoise_dir}/rooted-tree.qza \
		--i-table ${denoise_dir}/table.qza \
		--p-sampling-depth ${sample_depth} \
		--m-metadata-file ${metadata_file} \
		--output-dir ${denoise_dir}/core-metrics-results
else
	echo "Alpha and beta diversity analysis has been done, skipping ..."
fi
...
if [ ! -f ${denoise_dir}/core-metrics-results/faith-pd-group-significance.qzv ];then
	qiime diversity alpha-group-significance \
		--i-alpha-diversity ${denoise_dir}/core-metrics-results/faith_pd_vector.qza \
		--m-metadata-file ${metadata_file} \
		--o-visualization ${denoise_dir}/core-metrics-results/faith-pd-group-significance.qzv
else
	echo "faith_pd index has been done, skipping ..."
fi


#Rarefication curve
echo "doing rarefication curve ..."
if [ ! -f ${denoise_dir}/alpha-rarefaction.qzv ];then
	qiime diversity alpha-rarefaction \
		--i-table ${denoise_dir}/table.qza \
		--i-phylogeny ${denoise_dir}/rooted-tree.qza \
		--p-max-depth 5000 \
		--m-metadata-file ${metadata_file} \
		--o-visualization ${denoise_dir}/alpha-rarefaction.qzv
else
	echo "rarefication curve has been analized, skipping ..."
fi
